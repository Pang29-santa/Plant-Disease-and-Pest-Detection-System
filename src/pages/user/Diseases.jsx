...
const Diseases = () => {
  ...
  useEffect(() => {
    if (selectedDisease) {
      ...
      setActiveModalImg(selectedDisease.image_path);
    } else {
          ...
          <div className="max-w-xl mx-auto relative group">
            ...
            <div className="relative">
              <input
                ...
                className="w-full pl-14 pr-6 py-4 sm:py-5 bg-gray-50 border border-gray-100 rounded-2xl sm:rounded-3xl text-gray-900 placeholder-gray-400 font-bold focus:outline-none focus:bg-white focus:border-red-500 focus:ring-4 focus:ring-red-500/10 transition-all text-base shadow-sm"
              />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-8 relative z-20">
        {loading ? (
          ...
          <div className="bg-white rounded-[3rem] p-16 text-center max-w-md mx-auto shadow-xl shadow-gray-100 border border-gray-100">
            ...
            <h3 className="text-xl font-bold text-gray-900 mb-2">{t('diseasesPage.notFound')}</h3>
            <p className="text-gray-500 text-sm font-medium">{t('diseasesPage.trySearch')}</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredDiseases.map((disease) => (
              <div
                key={disease._id || disease.id || disease.thai_name}
                onClick={() => openDetail(disease._id || disease.id || disease)}
                className="group bg-white rounded-[2.5rem] overflow-hidden shadow-xl shadow-gray-200/50 hover:shadow-2xl hover:shadow-red-500/10 transition-all duration-500 border border-gray-100 flex flex-col cursor-pointer transform hover:-translate-y-1"
              >
                {/* Image Container */}
                <div className="relative aspect-[4/3] overflow-hidden bg-gray-900">
                  ...
                  <div className="absolute top-4 left-4">
                    <span className="px-3.5 py-1.5 bg-red-500/80 backdrop-blur-md text-white text-[10px] font-black uppercase tracking-widest rounded-xl border border-white/20 shadow-lg">
                      Disease
                    </span>
                  </div>
          ...
          <div className="relative w-full max-w-5xl bg-white rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col md:flex-row h-[88vh] md:h-[85vh] animate-in zoom-in-95 duration-300 border border-white/20">
            ...
            <div className="w-full md:w-5/12 bg-slate-950 relative flex flex-col justify-between p-6 md:p-8 shrink-0 min-h-[220px] md:min-h-full">
              ...
              <div className="absolute inset-0 overflow-hidden">
                <img
                  src={getImageUrl(activeModalImg || selectedDisease.image_path)}
              ...
              <div className="relative z-10">
                <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-red-500/20 backdrop-blur-md text-red-300 border border-red-500/30 rounded-full text-xs font-bold shadow-sm">
                  <Activity className="w-3.5 h-3.5 text-red-400 animate-pulse" />
                  <span>{selectedDisease.severity || t('detectPage.severity.medium')} Danger</span>
                </div>
              </div>
                ...
                {selectedDisease.image_paths && selectedDisease.image_paths.length > 1 && (
                  <div className="pt-2">
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-1 custom-scrollbar">
                      {selectedDisease.image_paths.map((path, idx) => (
                        <button
                          key={idx}
                          onClick={() => setActiveModalImg(path)}
                          className={`w-12 h-12 rounded-xl overflow-hidden border-2 transition-all shrink-0 ${
                            (activeModalImg || selectedDisease.image_path) === path 
                              ? 'border-red-500 scale-105 shadow-md shadow-red-500/40' 
            ...
            <div className="w-full md:w-7/12 bg-white flex-1 overflow-y-auto custom-scrollbar p-6 md:p-10 space-y-6">
              ...
              <section className="bg-amber-50/60 rounded-3xl p-5 md:p-6 border border-amber-100/80 shadow-xs">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    <h3 className="font-black text-slate-900 text-lg tracking-tight">{t('detailPage.symptoms') || 'ลักษณะอาการ'}</h3>
                    <p className="text-[10px] font-bold text-amber-700 uppercase tracking-widest">{isThai ? 'อาการที่สังเกตได้บนพืช' : 'Observable Symptoms'}</p>
                  </div>
                </div>
                <div 
              ...
              <section className="bg-rose-50/60 rounded-3xl p-5 md:p-6 border border-rose-100/80 shadow-xs">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    <h3 className="font-black text-slate-900 text-lg tracking-tight">{t('detailPage.cause') || 'สาเหตุของโรค'}</h3>
                    <p className="text-[10px] font-bold text-rose-700 uppercase tracking-widest">{isThai ? 'ปัจจัยและเชื้อสาเหตุ' : 'Pathogen & Conditions'}</p>
                  </div>
                </div>
                <div 
              ...
              <section className="bg-emerald-50/60 rounded-3xl p-5 md:p-6 border border-emerald-100/80 shadow-xs">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    <h3 className="font-black text-slate-900 text-lg tracking-tight">{t('detailPage.treatment') || 'แนวทางรักษา'}</h3>
                    <p className="text-[10px] font-bold text-emerald-700 uppercase tracking-widest">{isThai ? 'วิธีการจัดการโรคพืช' : 'Control & Treatment Steps'}</p>
                  </div>
                </div>
                <div 
              ...
              <section className="bg-sky-50/60 rounded-3xl p-5 md:p-6 border border-sky-100/80 shadow-xs">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    <h3 className="font-black text-slate-900 text-lg tracking-tight">{t('detailPage.prevention') || 'การป้องกัน'}</h3>
                    <p className="text-[10px] font-bold text-sky-700 uppercase tracking-widest">{isThai ? 'การดูแลและป้องกันล่วงหน้า' : 'Preventive Measures'}</p>
                  </div>
                </div>
                <div 
